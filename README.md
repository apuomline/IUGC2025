...
3. **关键点检测天然不适配基于 Softmax 最大值的伪标签筛选** —— 热图响应强度与实际定位精度严重偏离：高响应可能出现在错误位置（假阳性），而定位准确的点却可能因图像噪声、低对比度导致响应偏低而被错误过滤。

## 解决思路总览

我们的方案分为两个阶段：**先建立尽可能强的全监督基线，再以 Noisy Student 半监督框架提取未标注数据的价值**。
