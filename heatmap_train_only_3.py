...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=50, device='cuda', writer=None, save_dir=None, args=None):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, heatmaps, landmarks) in enumerate(train_loop):
            ...
            # Calculate distance
            with torch.no_grad():
                pred_coords = extract_coordinates(outputs)
        ...
        # Save best validation models
        if save_dir:
            # Save best validation distance model
...
def main(args):
    ...
    if 'pvt' in args.model_name.lower():
        ...
        model = densenet_unet(3, 3, heatmap_size=args.heatmap_size, arch=args.arch,
        pretrained=True,pretrained_path=r'pretrained_models\densenet121_tv_in1k_feature_only.pth')
        print(f"Using densenetunet model with architecture: {args.arch}")

    elif 'seresnext26d_unet' in args.model_name.lower():
        model = seresnext26d_unet(3, 3, heatmap_size=args.heatmap_size, arch=args.arch,
    ...
    elif 'resnet_unet' in args.model_name.lower():
        model = ResNet18_UNet(3, 3, heatmap_size=args.heatmap_size,
            pretrained=True)
        print(f"Using densenet_unet model with architecture: {args.arch}")
...
if __name__ == '__main__':
    ...
    """
    # 添加配置文件参数
    parser.add_argument('--config', type=str, default=r'config\densenet121_unet.yaml',
