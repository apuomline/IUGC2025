...
from models.pvt_unet_sup import pvt_unet_sup

from models.convnextv2_unet import convnext_unet
from models.densenet_unet import densenet_unet
from models.seresnext26d_unet import seresnext26d_unet
...
def update_args_from_config(args, config):
    ...
    param_mapping = {
        ...
        'mixup_prob': 'mixup_prob',  # 添加MixUp概率参数映射
        'mixup_final_prob': 'mixup_final_prob'  # 添加MixUp最终概率参数映射
    }
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=50, device='cuda', writer=None, save_dir=None, args=None):
    ...
    if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
        initial_mixup_prob = args.mixup_prob
        final_mixup_prob = args.mixup_final_prob  # 最终概率设为0，即不使用MixUp
        mixup_decay_rate = (initial_mixup_prob - final_mixup_prob) / num_epochs
        print(f"MixUp probability will linearly decay from {initial_mixup_prob:.3f} to {final_mixup_prob:.3f}")
        print(f"Decay rate per epoch: {mixup_decay_rate:.4f}")
    
    for epoch in range(num_epochs):
        # 计算当前epoch的MixUp概率
        if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
            current_mixup_prob = max(0.0, initial_mixup_prob - mixup_decay_rate * epoch)
        else:
        ...
        for i, (images, heatmaps, landmarks) in enumerate(train_loop):
            ...
            # 根据当前epoch的概率决定是否进行MixUp
            if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0 and random.random() < current_mixup_prob:
                # 进行MixUp操作
                ...
                # 记录MixUp使用情况
                mixup_used = 1.0
                mixup_lambda = mix_weights[0, 0].item()
            else:
                # 正常训练，不使用MixUp
                outputs = model(images)
                # 如果模型输出是双倍通道（用于MixUp），只取前K通道进行正常训练
                if outputs.shape[1] == 2 * num_keypoints:
                    outputs = outputs[:, :num_keypoints, :, :]  # 只取前K通道
            ...
            # 记录MixUp使用情况
            if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
                writer.add_scalar('MixUp/used', mixup_used, iteration)
                writer.add_scalar('MixUp/current_prob', current_mixup_prob, iteration)
                # 如果使用了MixUp，记录lambda值
                if mixup_used == 1.0:
                    writer.add_scalar('MixUp/lambda', mixup_lambda, iteration)
        ...
        with torch.no_grad():
            for i, (images, heatmaps, landmarks) in enumerate(val_loop):
                ...
                K = num_keypoints
                # 如果模型输出是双倍通道（用于MixUp），只取前K通道进行验证
        ...
        # 记录当前epoch的MixUp概率
        if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
            writer.add_scalar('MixUp/epoch_prob', current_mixup_prob, epoch)
        ...
        if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
            ...
            print(f'Current MixUp Probability: {current_mixup_prob:.3f}')
        print('=' * 50)
        if save_dir:
...
def main(args):
    ...
    # 根据MixUp配置决定输出通道数
    if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
        out_channels = num_keypoints * 2  # MixUp时需要双倍通道
    else:
        out_channels = num_keypoints  # 正常训练时单倍通道
    # Create model
    if 'pvt' in args.model_name.lower():
        if 'pvt_sup' in args.model_name.lower():

            model = pvt_unet_sup(3, out_channels, heatmap_size=args.heatmap_size, arch=args.arch)
        else:      
    ...
    elif 'seresnext26d_unet' in args.model_name.lower():
        model = seresnext26d_unet(3, out_channels, heatmap_size=args.heatmap_size, arch=args.arch,
            pretrained=True,pretrained_path=r'pretrained_models\seresnext26d_32x4d_feature_only.pth')
        print(f"Using densenet_unet model with architecture: {args.arch}")
    ...
    print(f"MixUp enabled: {hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0}")
    if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
        print(f"MixUp probability: {args.mixup_prob}")
    ...
    if hasattr(args, 'mixup_alpha') and args.mixup_alpha > 0:
        ...
        print(f"Initial MixUp probability: {args.mixup_prob}")
        print(f"MixUp probability will linearly decay to {args.mixup_final_prob:.3f} over {args.epochs} epochs")
...
if __name__ == '__main__':
    ...
    """
    # 添加配置文件参数
    parser.add_argument('--config', type=str, default=r'config\densenet_121_unet_prob.yaml',
    ...
    parser.add_argument('--mixup_prob', type=float, default=0.5,
                        help='Initial probability of applying MixUp data augmentation (0.0 to 1.0)')
    parser.add_argument('--mixup_final_prob', type=float, default=0.0,
                        help='Final probability of applying MixUp data augmentation after linear decay (0.0 to 1.0)')
